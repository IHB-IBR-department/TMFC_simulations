...
for subji = 1:N
    VOI = load([stat_path filesep exp_folder filesep 'GLMs' filesep 'Sub_' num2str(subji,'%.3d') filesep  'VOI_ALL.mat']);
    
    outputdir = [stat_path filesep exp_folder filesep 'GLMs' filesep 'Sub_' num2str(subji,'%.3d') filesep  'GLMsingle_cHRF'];
    ...
    load([stat_path filesep exp_folder filesep 'GLMs' filesep 'Sub_' num2str(subji,'%.3d') filesep 'GLMsingle_cHRF' filesep 'TYPED_FITHRF_GLMDENOISE_RR.mat'], 'modelmd');
    ...
    save([stat_path filesep exp_folder filesep 'GLMs' filesep 'Sub_' num2str(subji,'%.3d') filesep 'BSC_FRR.mat'],'BSC*');
    clear BSC* betas*
    fprintf(['BSC-FRR :: Subject #' num2str(subji) ' :: Done in: ' num2str(toc) 's \n']);
end
...
for subji = 1:N
    load([stat_path filesep exp_folder filesep 'GLMs' filesep 'Sub_' num2str(subji,'%.3d') filesep 'BSC_FRR.mat']);
...
time = toc;
fprintf(['Merge BSC-FRR matrices :: Done in: ' num2str(time) 's \n']); 
...
%% Save results
save([stat_path filesep exp_folder filesep 'group_stat' filesep 'BSC_FRR_RESULTS.mat'],'TPR*','TNR*');
