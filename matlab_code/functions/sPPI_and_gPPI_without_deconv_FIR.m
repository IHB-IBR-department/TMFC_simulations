function sPPI_and_gPPI_without_deconv_FIR(stat_path,exp_folder,N,N_ROIs,q_level,ground_truth)
...
for subji = 1:N
    ...
    for j = 1:N_ROIs    
        PPI_TaskA_vs_TaskB = detrend(VOI(:,j),'constant').*detrend(Psy_TaskA_vs_TaskB,'constant');
        PPI_TaskA = detrend(VOI(:,j),'constant').*detrend(Psy_TaskA,'constant');
        PPI_TaskB = detrend(VOI(:,j),'constant').*detrend(Psy_TaskB,'constant'); 
        
        sPPI_design = [detrend(PPI_TaskA_vs_TaskB,'constant') VOI(:,j) Psy_TaskA_vs_TaskB ones(size(Psy_TaskA_vs_TaskB))];
        gPPI_design = [detrend(PPI_TaskA,'constant') detrend(PPI_TaskB,'constant') VOI(:,j) Psy_TaskA Psy_TaskB ones(size(Psy_TaskA))];
    ...
    save([stat_path filesep exp_folder filesep 'GLMs' filesep 'Sub_' num2str(subji,'%.3d') filesep 'sPPI_and_gPPI_without_Deconv_FIR.mat'],...
          'beta_*', 'con_*', 'task_indep_*');
    time(subji)=toc;
    fprintf(['sPPI and gPPI without deconvolution :: Subject #' num2str(subji) ' :: Done in: ' num2str(time(subji)) 's \n']);     
...
for subji = 1:N
    load([stat_path filesep exp_folder filesep 'GLMs' filesep 'Sub_' num2str(subji,'%.3d') filesep 'sPPI_and_gPPI_without_Deconv_FIR.mat']);
...
save([stat_path filesep exp_folder filesep 'group_stat' filesep 'sPPI_and_gPPI_without_Deconv_FIR.mat'],'sPPI*','gPPI*');
time = toc;
fprintf(['Merge sPPI_and_gPPI_without_Deconv matrices :: Done in: ' num2str(time) 's \n']); 
...
%% Save results
save([stat_path filesep exp_folder filesep 'group_stat' filesep 'sPPI_and_gPPI_without_Deconv_FIR_RESULTS.mat'],'TPR*','TNR*','*symmetry');