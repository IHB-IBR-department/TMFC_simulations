function cPPI_without_deconv_FIR(stat_path,exp_folder,N,N_ROIs,q_level,ground_truth)
...
for subji = 1:N
    ...
    for j = 1:N_ROIs
        for k = j+1:N_ROIs
            ...
            PPI_TaskA_vs_TaskB_ROI1 = detrend(VOI(:,j),'constant').*detrend(Psy_TaskA_vs_TaskB,'constant');
            PPI_TaskA_vs_TaskB_ROI2 = detrend(VOI(:,k),'constant').*detrend(Psy_TaskA_vs_TaskB,'constant');
    ...
    save([stat_path filesep exp_folder filesep 'GLMs' filesep 'Sub_' num2str(subji,'%.3d') filesep 'cPPI_without_Deconv_FIR.mat'],'partcorr_TaskA_vs_TaskB_cPPI');
    time(subji)=toc;
...
for subji = 1:N
    load([stat_path filesep exp_folder filesep 'GLMs' filesep 'Sub_' num2str(subji,'%.3d') filesep 'cPPI_without_Deconv_FIR.mat']);
    cPPI_WoD_TaskA_vs_TaskB(:,:,subji) = partcorr_TaskA_vs_TaskB_cPPI;
end
save([stat_path filesep exp_folder filesep 'group_stat' filesep 'cPPI_without_Deconv_FIR.mat'],'cPPI*');
time = toc;
...
sgtitle('cPPI without Deconvolution (After FIR task regression)')
...
%% Save results
save([stat_path filesep exp_folder filesep 'group_stat' filesep 'cPPI_without_Deconv_RESULTS_FIR.mat'],'TPR*','TNR*');
